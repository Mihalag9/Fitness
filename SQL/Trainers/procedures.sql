...
BEGIN
    ...
    WHERE (p_fullname IS NULL OR t."FullName" ILIKE '%' || p_fullname || '%')
      -- Если p_no_experience IS TRUE, фильтруем, иначе игнорируем фильтр
      AND (CASE WHEN p_no_experience = TRUE THEN (t."Experience" IS NULL OR t."Experience" = 0) ELSE TRUE END)
    ORDER BY
        CASE WHEN p_sort_experience = 'asc' THEN t."Experience" END ASC NULLS LAST,
        CASE WHEN p_sort_experience = 'desc' THEN t."Experience" END DESC NULLS LAST,
        t."FullName" ASC;
END;
$$ LANGUAGE plpgsql;
